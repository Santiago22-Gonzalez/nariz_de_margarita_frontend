...
(function(){
  ...
  const loginOpenButtons = document.querySelectorAll('.user-action.btn-primary, .open-login, .login-link');
  ...
  if (modalOverlay) modalOverlay.addEventListener('click', closeLogin);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const cartSubtotalEl = cartDrawer ? cartDrawer.querySelector('.cart-subtotal') : null;
  ...
  function addToCart(product) {
    ...
    const existing = Array.from(cartItemsList.querySelectorAll('.cart-item')).find(li => li.getAttribute('data-id') === product.id);
    if (existing) {
      ...
      li.innerHTML = `
        ...
        <div style="flex:1;">
          ...
          <div style="color:#7a7a7a;font-size:0.95rem">${formatCOP(product.price)}</div>
        </div>
  ...
  document.querySelectorAll('.add-cart').forEach(btn => {
    btn.addEventListener('click', function (e) {
      ...
      if (!card) return;
      const name = (card.querySelector('.product-name')?.textContent || 'Producto').trim();
      const priceText = (card.querySelector('.price')?.textContent || card.querySelector('.old-price')?.textContent || '0');
      const price = parsePrice(priceText);
      const img = card.querySelector('img') ? card.querySelector('img').src : '';
      // id único por nombre+precio (puedes usar data-id en producto para mejor resultado)
      const id = (card.getAttribute('data-id') || (name + '|' + price)).replace(/\s+/g, '-').toLowerCase();
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function buildMessage() {
    ...
    if (cartItemsList) {
      cartItemsList.querySelectorAll('.cart-item').forEach(li => {
        const name = (li.querySelector('div > div:first-child')?.textContent || li.querySelector('.product-name')?.textContent || li.getAttribute('data-id') || 'Producto').trim();
        const qty = li.getAttribute('data-qty') || li.querySelector('.cart-qty')?.textContent || '1';
        items.push(`${name} x${qty}`);
      });
    }
    const itemsText = items.length ? ` Productos: ${items.join(', ')}.` : '';
    return `Hola, quiero finalizar mi compra.${subtotalText ? ' Subtotal: ' + subtotalText + '.' : ''}${itemsText}`;
  }
  ...
  updateHref();
  if (subtotalEl) {
    new MutationObserver(updateHref).observe(subtotalEl, { childList: true, characterData: true, subtree: true });
  }
  if (cartItemsList) {
    new MutationObserver(updateHref).observe(cartItemsList, { childList: true, subtree: true });
  }
...
(function(){
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && registerModal.getAttribute('aria-hidden') === 'false') closeRegister();
  });
